...
class RRT(object):
    ...
    def scan_callback(self, scan_msg):
        ...
        while self.is_goal(latest_node) == False:
            ...
            if self.check_collision(self.tree[nearest_node_index],new_node):
                ...
                new_node_index = len(self.tree) #
                self.tree.append(new_node)
                
                
                min_node_index = nearest_node_index #x_min = x_nearest
                c_min = self.cost(nearest_node_index) + self.line_cost(self.tree[nearest_node_index], new_node)
                
                for near_node_index in neighborhood:
                    if self.check_collision(self.tree[near_node_index], new_node):
                        c = self.cost(near_node_index) + self.line_cost(self.tree[near_node_index],new_node )
                        if c < c_min:
                            min_node_index = near_node_index
                            c_min = c
                            
                self.tree[new_node_index].parent = min_node_index # add x_min -> x_new
                
                for near_node_index in neighborhood:
                    if self.check_collision(self.tree[near_node_index], new_node):
                        c = self.cost(new_node_index) + self.line_cost(self.tree[near_node_index], new_node)
                        if c < self.cost(near_node_index):
                            self.tree[new_node_index].parent = near_node_index
                            
                latest_node = self.tree[new_node_index]
            print("iteration : ",i)
            if(i>500) : break
    ...
    def cost(self, nd_index):
        ...
        """
        nd = self.tree[nd_index]
        
        """
        while(nd.is_root==False):
            print("cost")
            nd_parent = self.tree[nd.parent]
            cout += self.dist_euclid(nd.x,nd.y,nd_parent.x,nd_parent.y)
            nd = nd_parent
        """
        return self.dist_euclid(nd.x,nd.y,self.x,self.y)
    ...
    def line_cost(self, n1, n2):
        ...
        """
        return self.dist_euclid(n1.x, n1.y, n2.x, n2.y)
    ...
    def near(self, node):
        ...
        neighborhood = []
        m = ARM_LENGHT+0.3
        for i, test_node in enumerate(self.tree):
            if self.line_cost(test_node, node) < m:
